...
if not _httplib:
  ...
  try:
    ...
    if (major, minor, patch) < (0, 8, 8):
      print('Warning: the Stripe library requires that your Python "requests" library has a version no older than 0.8.8, but your "requests" library has version %s. Stripe will fall back to an alternate HTTP library, so everything should work, though we recommend upgrading your "requests" library. If you have any questions, please contact support@stripe.com. (HINT: running "pip install -U requests" should upgrade your requests library to the latest version.)' % (version, ), file=sys.stderr)
      _httplib = None
...
if not _httplib:
  try:
    ...
    _httplib = 'urllib2'
    print("Warning: the Stripe library is falling back to urllib2 because neither requests nor pycurl are installed. urllib2's SSL implementation doesn't verify server certificates. For improved security, we suggest installing requests.", file=sys.stderr)
  except ImportError:
    pass
...
class APIRequestor(object):
  ...
  def encode(cls, d):
    ...
    """
    return urllib.parse.urlencode(cls._encode_inner(d))
